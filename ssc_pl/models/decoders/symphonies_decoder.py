...
from ..layers import (ASPP, DeformableSqueezeAttention, DeformableTransformerLayer,
                      LearnableSqueezePositionalEncoding, TransformerLayer, Upsample, MultiLevelFeatureSampler,
                      RegionAttentionModule, CrossAttention)
...
class SymphoniesDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.VP_sampler = MultiLevelFeatureSampler(embed_dims).to('cuda')
        self.RegionAttn = RegionAttentionModule(dim=embed_dims, kernel_size=5)

        self.cross_VP2inst = CrossAttention(embed_dim=embed_dims, num_heads=8)
        self.cross_inst2VP = CrossAttention(embed_dim=embed_dims, num_heads=8)
        self.cross_inst2VP＿init = CrossAttention(embed_dim=embed_dims, num_heads=8)

    @autocast(dtype=torch.float32)
    def forward(self, pred_insts, feats, pred_masks, depth, K, E, voxel_origin, projected_pix,
                fov_mask, vp, vp_ref):
        
        center = torch.tensor([0.5, 0.55]).to('cuda')
        ...
        #     print(f'{i} feat shape: ', feat.shape)

        VP_queries = self.VP_sampler(vp_ref, feats)
        VP_queries = torch.mean(VP_queries, 1)
        VP_queries = self.cross_inst2VP＿init(VP_queries, inst_queries, inst_queries)
        #   (bs, n, c)

        # Depth shape:  torch.Size([1, 370, 1220])
        # 0 feat shape:  torch.Size([1, 128, 93, 305])
        # 1 feat shape:  torch.Size([1, 128, 47, 153])
        # 2 feat shape:  torch.Size([1, 128, 24, 77])
        ...
        for i, layer in enumerate(self.layers):
            VP_queries = self.RegionAttn(feats[0], center, vp_ref, VP_queries)
            VP_queries = self.cross_inst2VP(VP_queries, inst_queries, inst_queries)
            inst_queries = self.cross_VP2inst(inst_queries, VP_queries, VP_queries)
